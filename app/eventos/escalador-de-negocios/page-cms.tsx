...
export default function EscaladorDeNegociosPageCMS() {
  ...
  useEffect(() => {
    ...
    const fetchEventData = async () => {
      try {
        const data = await getEvent('escalador-de-negocios')
        setEventData(data)
      } catch (error) {
  ...
  const eventTitle = eventData?.title || "ESCALADOR DE NEGÓCIOS"
  const eventSubtitle = eventData?.subtitle || "Evento Presencial Exclusivo"
  const eventSecondTitle = eventData?.secondTitle || "Empreendedores de sucesso não crescem por acaso"
  const eventDescription = eventData?.description || "Saia da estagnação e aplique, de forma imediata, estratégias reais para escalar vendas, lucros e liberdade."
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white">
      ...
      {/* What You Will Learn Section - IDÊNTICO AO ORIGINAL */}
      ...
      {/* Event Highlights Section - IDÊNTICO AO ORIGINAL */}
      <section className="py-20 relative bg-zinc-900/40">
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 bg-zinc-800/50 backdrop-blur-sm border border-zinc-700/50 rounded-full py-2 px-4 mb-4">
      ...
      {/* Mentor Section - IDÊNTICO AO ORIGINAL */}
      <MentorSection />
