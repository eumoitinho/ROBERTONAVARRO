...
interface EnergiaDodinheiroPageClientProps {
  data: EventPageData
}

export default function EnergiaDodinheiroPageClient({ data }: EnergiaDodinheiroPageClientProps) {
  ...
  return (
     <div className="min-h-screen bg-gradient-to-b from-zinc-950 via-zinc-900 to-zinc-800 text-white relative overflow-x-hidden">
      ...
      {/* Hero Section */}
      <HeroPages
        title={data.hero?.title || "ENERGIA DO DINHEIRO"}
        subtitle={data.hero?.subtitle || "Desbloqueie a energia do dinheiro e transforme sua realidade"}
        secondtitle={data.hero?.secondTitle || "07 de Outubro - Das 13h às 20h"}
        description={data.hero?.description || "Alinhe sua energia com a prosperidade e conquiste abundância real na vida e nos negócios. Este evento não entrega apenas conhecimento, mas vivências profundas que desbloqueiam crenças, dissolvem padrões limitantes e ativam a força interna da prosperidade."}
        image={data.hero?.image?.asset?.url || "/images/HERO_ENERGIA.png"}
        ctaText={data.hero?.ctaText || "GARANTA SUA VAGA!"}
      ...
      <section id="beneficios" className="py-20 relative">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center mb-16">
            <SectionBadge text={data.benefitsSection?.badge || "QUAIS BLOQUEIOS TE AFASTAM DA RIQUEZA"} />
            <h2 className="text-2xl md:text-3xl font-bold mb-4 uppercase">
              {data.benefitsSection?.title || "Descubra os sabotadores invisíveis que drenam sua energia financeira"}
            </h2>
          </div>
...
<MentorSection />
      ...
      <NewsletterFormacoes
        source={data.newsletterSection?.source || "Energia do Dinheiro"}
        title={data.newsletterSection?.title || "GARANTA SUA VAGA NO ENERGIA DO DINHEIRO"}
        description={data.newsletterSection?.description || "Preencha o formulário abaixo e fique atento ao próximo evento"}
