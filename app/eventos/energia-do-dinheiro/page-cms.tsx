...
export default function EnergiaDoDinheiroPageCMS() {
  ...
  useEffect(() => {
    ...
    const fetchEventData = async () => {
      try {
        const data = await getEvent('energia-do-dinheiro')
        setEventData(data)
      } catch (error) {
  ...
  const eventTitle = eventData?.title || "ENERGIA DO DINHEIRO"
  const eventSubtitle = eventData?.subtitle || "Desbloqueie a energia do dinheiro e transforme sua realidade"
  const eventSecondTitle = eventData?.secondTitle || "Dois dias intensos de transformação para mudar sua relação com o dinheiro"
  const eventDescription = eventData?.description || "Alinhe sua energia com a prosperidade e conquiste abundância real na vida e nos negócios. Este evento não entrega apenas conhecimento, mas vivências profundas que desbloqueiam crenças, dissolvem padrões limitantes e ativam a força interna da prosperidade."
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-950 via-zinc-900 to-zinc-800 text-white relative overflow-x-hidden">
      {/* Padrão de ruído para o background - IDÊNTICO AO ORIGINAL */}
      <div className="fixed inset-0 noise-bg pointer-events-none"></div>
      
      {/* Elementos decorativos - IDÊNTICO AO ORIGINAL */}
      ...
      {/* Benefícios Section - IDÊNTICO AO ORIGINAL */}
      ...
      {/* Como Funciona Section - IDÊNTICO AO ORIGINAL */}
      ...
      {/* Modal de vídeo - IDÊNTICO AO ORIGINAL */}
      {activeVideoId && (
      ...
      <NewsletterSignup 
        ...
        title="SAIBA QUANDO HAVERÁ A ENERGIA DO DINHEIRO" 
        description="Inscreva-se para receber dicas e conteúdos exclusivos sobre finanças pessoais e investimentos." 
        onSubmit={() => {
