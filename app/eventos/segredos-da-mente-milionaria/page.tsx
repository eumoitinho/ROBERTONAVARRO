...
export async function generateMetadata(): Promise<Metadata> {
  const event = await getEventBySlug('segredos-da-mente-milionaria');
...
export default async function SegredosDaMenteMilionariaPage() {
  const event = await getEventBySlug('segredos-da-mente-milionaria');

  if (!event) {
    console.error('[Segredos da Mente Milionária] Evento não encontrado no Sanity');
    notFound();
  }
