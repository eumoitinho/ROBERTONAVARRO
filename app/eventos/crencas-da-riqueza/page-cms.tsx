...
export default function CrencasDaRiquezaPageCMS() {
  ...
  useEffect(() => {
    ...
    const fetchEventData = async () => {
      try {
        const data = await getEvent('crencas-da-riqueza')
        setEventData(data)
      } catch (error) {
    ...
    // Event listener para fechar o vídeo com a tecla ESC
  ...
  const eventTitle = eventData?.title || "CRENÇAS DA RIQUEZA"
  const eventSubtitle = eventData?.subtitle || "Transformação mental"
  const eventSecondTitle = eventData?.secondTitle || "A riqueza começa na mente e se materializa nas decisões"
  const eventDescription = eventData?.description || "Desbloqueie seu potencial, supere crenças limitantes e alcance um novo patamar de liberdade financeira e realização pessoal."
  const eventDate = eventData?.date ? new Date(eventData.date) : new Date("2025-06-27T09:00:00")
  const eventPrice = eventData?.price?.value ? `R$ ${eventData.price.value.toLocaleString('pt-BR')}` : "R$ 9,90"
  ...
  // Original static data preserved as fallbacks
  const challenges = [
    {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-950 via-zinc-900 to-zinc-800 text-white relative overflow-x-hidden">
      ...
      {/* What You'll Learn Section - IDÊNTICO AO ORIGINAL */}
      <section className="relative" id="o-que-aprender">
      ...
      {/* Event Highlights - IDÊNTICO AO ORIGINAL */}
      ...
      {/* Testimonials Section - IDÊNTICO AO ORIGINAL */}
      ...
      {/* Mentors Section - IDÊNTICO AO ORIGINAL */}
      ...
      {/* Form Section - LAYOUT ORIGINAL COM PREÇO DO SANITY */}
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="max-w-4xl mx-auto">
            <div className="bg-zinc-900/90 border-2 border-yellow-500/40 rounded-3xl p-10 text-center">
              ...
              <p className="text-4xl font-extrabold text-yellow-400 mb-2">{eventPrice}</p>
              <p className="text-zinc-300 mb-6">Ingresso Especial - Vagas Limitadas</p>
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-8 text-left">
                <div>
                  ...
                  <ul className="space-y-2 text-zinc-300">
                    <li className="flex items-start gap-2">
                      ...
                      <span>Experiência completa de 10 horas</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <CheckCircle className="h-5 w-5 text-yellow-400 mt-0.5 flex-shrink-0" />
                ...
                <div>
                  ...
                  <ul className="space-y-2 text-zinc-300">
                    <li className="flex items-start gap-2">
                      <CheckCircle className="h-5 w-5 text-yellow-400 mt-0.5 flex-shrink-0" />
                      <span>Networking com participantes</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <CheckCircle className="h-5 w-5 text-yellow-400 mt-0.5 flex-shrink-0" />
                      <span>Acesso aos mentores</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <CheckCircle className="h-5 w-5 text-yellow-400 mt-0.5 flex-shrink-0" />
                      <span>Garantia de transformação</span>
                    </li>
                  </ul>
                </div>
              </div>
      ...
      {/* FAQ Section - IDÊNTICO AO ORIGINAL */}
      <section className="py-24 relative">
        <div className="absolute inset-0 bg-gradient-to-b from-zinc-900/95 to-zinc-950/95"></div>
        <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-yellow-400/30 to-transparent"></div>
        <div className="container mx-auto px-4 relative z-10">
      ...
      {/* WhatsApp Button */}
      <WhatsAppButton source="Crenças da Riqueza" />
    </div>
  )
}