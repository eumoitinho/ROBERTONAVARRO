...
const ctaLink = "#inscricao"

export default function MentorCoachingFinanceiroClient({ data }: Props) {
  const n = data
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
        ...
        <div className="container mx-auto px-4 relative z-10 flex flex-col justify-center">
          <ScrollAnimation animation="fadeInUp" className="max-w-4xl mx-auto text-center">
            <Badge variant="outline" className="mb-6 px-4 py-2 border-yellow-500/50 bg-yellow-500/5">
              <span className="text-sm font-medium">{n.hero?.subtitle || "MENTOR COACHING FINANCEIRO"}</span>
            </Badge>
            ...
            <p className="text-lg md:text-2xl text-zinc-300 mb-8 max-w-3xl mx-auto">
              {n.hero?.description || "Aprenda a instalar a inteligência financeira na sua vida e aumentar sua renda, com estratégias comprovadas."}
            </p>
            ...
            <Button
              ...
              asChild
            >
              <a href={n.hero?.ctaLink || ctaLink}>
                {n.hero?.ctaText || "ESTOU PRONTO PARA MUDAR MINHA VIDA!"}
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center max-w-6xl mx-auto">
            <ScrollAnimation animation="fadeInLeft">
              <div className="relative h-[500px] rounded-2xl overflow-hidden shadow-2xl">
                <Image
                  src={n.mentorSection?.image?.asset?.url || "/images/roberto-palestra.jpeg"}
            ...
            <ScrollAnimation animation="fadeInRight">
              <div>
                <Badge variant="outline" className="mb-4 px-4 py-2 border-yellow-500/50 bg-yellow-500/5">
                  {n.mentorSection?.badge || "TRANSFORME SUA VIDA"}
                </Badge>
                <h2 className="text-3xl md:text-4xl font-bold mb-6">
                  {n.mentorSection?.title || (
                ...
                <p className="text-zinc-300 text-lg mb-6">
                  {n.mentorSection?.paragraphs?.[0] || "Você já conquistou muito, mas sente que pode ir além? O Mentor Coaching Financeiro foi criado para profissionais como você, que desejam quebrar o teto de vidro financeiro e alcançar um novo patamar de prosperidade."}
                </p>
            ...
            <Card className="bg-gradient-to-br from-zinc-900 to-zinc-800 border-zinc-700 max-w-4xl mx-auto">
              <CardContent className="p-8 md:p-12">
                <ScrollAnimation animation="fadeInUp">
                  <Badge variant="outline" className="mb-6 px-4 py-2 border-yellow-500/50 bg-yellow-500/5">
                    {n.aboutSection?.badge || "MENTOR COACHING FINANCEIRO"}
                  </Badge>
                </ScrollAnimation>
                <ScrollAnimation animation="fadeInUp" animationDelay="100ms">
                  <h2 className="text-3xl md:text-4xl font-bold mb-6">
                    {n.aboutSection?.heading || (
                ...
                <div className="space-y-4 text-zinc-300">
                  {(n.aboutSection?.paragraphs || [
                    ...
                    <ScrollAnimation key={index} animation="fadeInUp" animationDelay={`${(index + 2) * 100}ms`}>
                      <p className={index === 1 ? "font-semibold text-yellow-400 text-lg" : ""}>{paragraph}</p>
                    </ScrollAnimation>
                  ))}
                </div>
              </CardContent>
      ...
      <section className="py-20 bg-zinc-900/30">
        <div className="container mx-auto px-4">
          <ScrollAnimation animation="fadeInUp" className="text-center mb-16">
            <Badge variant="outline" className="mb-4 px-4 py-2 border-yellow-500/50 bg-yellow-500/5">
              {n.learnSection?.badge || "CONTEÚDO"}
            </Badge>
            <h2 className="text-3xl md:text-4xl font-bold mb-4">
              {n.learnSection?.title || (
                <>O QUE VOCÊ{" "}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 to-amber-600">
                  APRENDERÁ?
                </span></>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {(n.learnSection?.items || learningModules).map((module: any, index: number) => (
                  ...
                  <CardContent className="p-6">
                    <div className="bg-yellow-500/10 rounded-full p-4 w-fit mb-4 mx-auto">
                      <div className="text-yellow-400">
                        {module.icon || <Brain className="h-8 w-8" />}
                      </div>
                    </div>
                    <h3 className="text-xl font-bold mb-3 text-center">{module.title}</h3>
                    <p className="text-zinc-300 text-sm text-center">{module.desc || module.description}</p>
                  </CardContent>
                </Card>
              </ScrollAnimation>
            ))}
      ...
      <section className="py-20 relative">
        <div className="container mx-auto px-4">
          <ScrollAnimation animation="fadeInUp" className="text-center mb-16">
            <Badge variant="outline" className="mb-4 px-4 py-2 border-yellow-500/50 bg-yellow-500/5">
              {n.audienceSection?.badge || "PÚBLICO-ALVO"}
            </Badge>
            <h2 className="text-3xl md:text-4xl font-bold mb-4">
              {n.audienceSection?.title || (
                <>PARA QUEM É O{" "}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 to-amber-600">
                  TREINAMENTO?
                </span></>
            ...
            <p className="text-zinc-300 max-w-3xl mx-auto">
              {n.audienceSection?.intro || "Este treinamento foi desenvolvido especificamente para:"}
            </p>
          </ScrollAnimation>

          <div className="space-y-6 max-w-5xl mx-auto">
            {(n.audienceSection?.bullets || targetAudience).map((audience: any, index: number) => (
              <ScrollAnimation key={index} animation="fadeInLeft" animationDelay={`${index * 100}ms`}>
                    ...
                    <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                      <div className="flex-shrink-0">
                        <div className="bg-yellow-500/10 rounded-full p-4 text-yellow-400">
                          {audience.icon || <Briefcase className="h-12 w-12" />}
                        </div>
                      </div>
                      <div className="text-center md:text-left">
                        <h3 className="text-xl font-bold mb-2 text-yellow-400">{audience.title}</h3>
                        <p className="text-zinc-300">{audience.desc || audience.description}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              <CardContent className="p-0">
                <div className="grid md:grid-cols-2 gap-8">
                  <ScrollAnimation animation="fadeInLeft" className="relative h-[400px] md:h-auto">
                    <Image
                      src={n.mentorSection?.image?.asset?.url || "/images/ROBERTO_6.jpg"}
                      alt="Roberto Navarro"
                      fill
                      className="object-cover"
                    />
                  ...
                  <div className="p-8 md:p-12">
                    ...
                    <ScrollAnimation animation="fadeInUp" animationDelay="200ms">
                      <p className="text-zinc-300 mb-6">
                        {n.mentorSection?.paragraphs?.[0] || "O maior e mais experiente formador de educadores, coaches e mentores financeiros do Brasil!"}
                      </p>
                    </ScrollAnimation>
                    ...
                    <div className="space-y-4 text-zinc-300">
                      {(n.mentorSection?.paragraphs?.slice(1) || [
                        ...
                        <ScrollAnimation key={index} animation="fadeInUp" animationDelay={`${(index + 4) * 100}ms`}>
                          <p>{paragraph}</p>
                        </ScrollAnimation>
                      ))}
                    </div>
      ...
      <section className="py-20 relative">
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {(n.benefits?.items || expectedResults).map((result: any, index: number) => (
              <ScrollAnimation key={index} animation="fadeInUp" animationDelay={`${index * 100}ms`}>
                <div className="bg-zinc-900/50 backdrop-blur-sm border border-zinc-800 rounded-xl p-6 hover:border-yellow-500/50 transition-all duration-300 hover:-translate-y-2">
                  ...
                  <p className="text-zinc-300 text-sm">{result.description || result.desc}</p>
                </div>
              </ScrollAnimation>
            ))}
          </div>
            ...
            <Card className="bg-gradient-to-br from-yellow-500/20 to-amber-600/20 border-yellow-500/50 max-w-4xl mx-auto">
              <CardContent className="p-8 md:p-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-6 text-yellow-400">
                  {n.guarantees?.items?.[0]?.title || "6 meses para experimentar uma mudança real"}
                </h3>
                <div className="space-y-4 text-lg">
                  <p>
                    {n.guarantees?.items?.[0]?.description || "Ao se inscrever no Mentor Coaching Financeiro, você conta com uma garantia incondicional de 6 meses. Aplique o método, veja resultados reais na sua vida financeira ou receba o dobro do seu dinheiro de volta!"}
                  </p>
