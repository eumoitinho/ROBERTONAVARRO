...
export default function WhatsappButton({ message, className, children, source }: WhatsappButtonProps) {
  ...
  // Read number from env (NEXT_PUBLIC_*) so it's available in client bundle
  ...
  const validateForm = (): string | null => {
    ...
    if (!name || name.length < 3) return "Por favor, insira um nome completo válido (mínimo 3 caracteres).";
    if (!email) return "Por favor, insira um email válido.";
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) return "Por favor, insira um email válido.";
    if (!phone || phone.length < 10) return "Por favor, insira um telefone válido (mínimo 10 dígitos).";
    return null;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (typeof window !== "undefined" && (window as any).dataLayer) {
          (window as any).dataLayer.push({ event: "complete_whatsapp", content_type: "whatsapp", category: "contato" });
        }
