...
export default function HeroPages({
  ...
  return (
    <section className="relative pt-32 md:pt-48 lg:pt-32 pb-16 md:pb-28 lg:pb-10 overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <Image
          ...
          alt={title}
          fill
          className="object-cover mt-10 md:mt-10"
          style={{ objectPosition: "right" }}
          priority
          sizes="100vw"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-black from-30% via-black/70 via-50% to-transparent"></div>
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center">
          <div className={`transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}>
            ...
            {description && (
              <p className="text-base md:text-xl text-zinc-300 mb-4 max-w-xl whitespace-pre-line">
                {description}
              </p>
            )}
            <div className="flex flex-col sm:flex-row gap-3 md:gap-4 mb-6 md:mb-8">
              <Button
                className="cta-hover bg-gradient-to-r from-yellow-500 to-amber-600 hover:from-yellow-600 hover:to-amber-700 text-black font-semibold rounded-full px-6 md:px-8 py-4 md:py-6 text-base"
                asChild
              >
              ...
              <Button
                asChild
                className="cta-hover-subtle bg-transparent hover:bg-zinc-800/50 border border-zinc-700 text-white font-medium rounded-full px-6 md:px-8 py-4 md:py-6 text-base"
              >
         ...
         {/* Stats */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 ">
            {[
              { icon: <Users className="h-6 w-6 text-yellow-400" />, value: "500+", label: "Mentorados" },
              { icon: <Star className="h-6 w-6 text-yellow-400" />, value: "4.9/5", label: "Avaliação" },
              { icon: <Zap className="h-6 w-6 text-yellow-400" />, value: "7+", label: "Anos de mentoria" },
              { icon: <Brain className="h-6 w-6 text-yellow-400" />, value: "100%", label: "Transformação" },
            ].map((stat, index) => (
              <div
                key={index}
