...
if (!PROJECT_ID || !DATASET || !TOKEN) {
  console.error('[populate-homepage] Faltam variáveis de ambiente: NEXT_PUBLIC_SANITY_PROJECT_ID, NEXT_PUBLIC_SANITY_DATASET e SANITY_API_WRITE_TOKEN')
  process.exit(1)
}

/** @type {import('node:tls').RequestOptions} */
const endpoint = `https://${PROJECT_ID}.api.sanity.io/v${API_VERSION}/data/mutate/${DATASET}`

/** Texto original conforme página atual (sem imagens) */
const doc = {
  _id: 'homepage',
  _type: 'homepage',
  ...
  videosSection: {
    ...
    videos: [
      { youtubeId: 'sVcR5iq1BG0', title: 'Estudo de Caso Fabio Santos - ICF', person: 'Fabio Santos', description: 'Relato de transformação financeira com o Instituto Coaching Financeiro.' },
      { youtubeId: 'AyjH3rNe37M', title: 'Estudo de Caso Clelio - ICF', person: 'Clelio', description: 'História de superação e sucesso com o Instituto Coaching Financeiro.' },
      { youtubeId: 'pmbpDqpkK78', title: 'Estudo de Caso Wagner Jovino - ICF', person: 'Wagner Jovino', description: 'Transformação financeira e pessoal com o Instituto Coaching Financeiro.' },
      { youtubeId: '7N97LDt9F5Y', title: 'Estudo de Caso Rodrigo - ICF', person: 'Rodrigo', description: 'Como Rodrigo transformou sua vida financeira com o ICF.' },
      { youtubeId: 'FUkJWtmjGtM', title: 'Depoimento João Leles - ICF', person: 'João Leles', description: 'Depoimento sobre a experiência com o Instituto Coaching Financeiro.' },
      { youtubeId: 'Jyokxvo-WOo', title: 'Depoimento Ricardo - ICF', person: 'Ricardo', description: 'Relato de transformação financeira com o Instituto Coaching Financeiro.' },
      { youtubeId: 'GQVv0wnK4So', title: 'Como Roosevelt transformou sua vida financeira - ICF', person: 'Roosevelt', description: 'História de sucesso com o Instituto Coaching Financeiro.' },
      { youtubeId: 'kfZ-hck8bJI', title: 'Depoimentos - Instituto Coaching Financeiro - ICF', person: 'Vários Participantes', description: 'Compilado de depoimentos sobre o impacto do Instituto Coaching Financeiro.' },
  ...
  testimonialsSection: {
    ...
    testimonials: [
      { name: 'Ana Souza', role: 'Empresária', initial: 'A', quote: 'Eu estava completamente endividada, sem esperança de sair do vermelho. O método do Roberto me ajudou a organizar minhas finanças, quitar dívidas e voltar a sonhar. Hoje tenho controle e paz financeira!', rating: 5 },
      { name: 'José Lima', role: 'Professor', initial: 'J', quote: 'Sempre achei impossível sair das dívidas do cartão. Com as orientações do Roberto, consegui renegociar tudo, criar uma reserva e até investir. Minha vida mudou completamente.', rating: 5 },
      { name: 'Patrícia Gomes', role: 'Autônoma', initial: 'P', quote: 'O Roberto me mostrou que é possível recomeçar. Saí do sufoco das dívidas, aprendi a gastar com consciência e hoje ajudo minha família a ter uma vida mais tranquila.', rating: 5 },
    ],
  ...
  seo: {
    ...
    keywords: ['Roberto Navarro','Coaching Financeiro','Mentalidade','Transformação Financeira','Educação Financeira','Mentor Financeiro'],
  },
}
...
async function main() {
  ...
  if (!res.ok) {
    ...
    console.error('[populate-homepage] Falha:', res.status, txt)
    process.exit(1)
  }
  const json = await res.json()
  console.log('[populate-homepage] OK', JSON.stringify(json, null, 2))
}

main().catch((err) => {
  console.error('[populate-homepage] Erro inesperado', err)
  process.exit(1)
})
